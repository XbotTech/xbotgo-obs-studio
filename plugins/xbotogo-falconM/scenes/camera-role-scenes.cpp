#include "camera-role-scenes.hpp"
 ...
 */
template<typename Callback> void EnumSceneCollectionScenes(Callback callback, void *param)
{
	...
	obs_enum_canvases(
		[](void *context, obs_canvas_t *canvas) {
			auto &data = *static_cast<CanvasData *>(context);
			if (!(obs_canvas_get_flags(canvas) & EPHEMERAL)) {
				obs_canvas_enum_scenes(canvas, data.callback, data.param);
			}
			return true;
		},
		&data);
...
OBSSource FindMarkedCameraRoleScene(const CameraRoleSceneConfig &config)
{
...
OBSSource FindCameraRoleScene(const CameraRoleSceneConfig &config)
{
	OBSSource sceneSource = FindMarkedCameraRoleScene(config);
...
OBSSource GetOrCreateCameraRoleScene(const CameraRoleSceneConfig &config)
{
	OBSSource sceneSource = FindCameraRoleScene(config);
	if (!sceneSource) {
...
std::vector<OBSSceneItem> FindSourceSceneItems(obs_source_t *source)
{
...
{
	const CameraRoleSceneConfig *config = FindCameraRoleSceneConfig(role);
	return config ? FindCameraRoleScene(*config) : OBSSource{};
}
...
std::optional<CameraRole> GetSourceCameraRole(obs_source_t *source)
{
	...
	const std::vector<OBSSceneItem> sourceItems = FindSourceSceneItems(source);
	for (const auto &config : CameraRoleScenes) {
		OBSSource sceneSource = FindCameraRoleScene(config);
...
bool AssignSourceToCameraRoleScene(obs_source_t *source, CameraRole role)
{
	...
	OBSSource targetSceneSource = GetOrCreateCameraRoleScene(*config);
	OBSScene targetScene = targetSceneSource ? obs_scene_from_source(targetSceneSource) : nullptr;
	if (!targetScene) {
		return false;
	}

	std::vector<OBSSceneItem> sourceItems = FindSourceSceneItems(source);
	...
	if (!targetItem) {
		OBSSourceAutoRelease currentSceneSource = obs_frontend_get_current_scene();
		OBSScene currentScene = currentSceneSource ? obs_scene_from_source(currentSceneSource) : nullptr;
