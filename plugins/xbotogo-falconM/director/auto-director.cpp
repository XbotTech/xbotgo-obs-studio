...
void AutoDirector::start()
{
	...
	XBLOG_INFO("XBotGo auto director started");
}
...
void AutoDirector::stop()
{
	...
	XBLOG_INFO("XBotGo auto director stopped");
}
...
void AutoDirector::setCenterConfigurationState(CenterConfigurationState state)
{
	...
	if (state == CenterConfigurationState::Missing) {
		XBLOG_WARNING("XBotGo auto director paused: center camera is not configured");
	} else if (state == CenterConfigurationState::Multiple) {
		XBLOG_WARNING("XBotGo auto director paused: multiple center cameras are configured");
	} else if (state == CenterConfigurationState::Valid &&
		   centerConfigurationState_ != CenterConfigurationState::Unknown) {
		XBLOG_INFO("XBotGo auto director resumed: center camera configuration is valid");
	}
				     ...
				     std::chrono::steady_clock::time_point reportedAt)
{
	...
	XBLOG_INFO("XBotGo auto director switched program to %s camera at horizontal angle %.2f",
