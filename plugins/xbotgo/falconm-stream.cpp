...
public:
	...
	{
		XBLOG_INFO("this=%p uniqueID=%d FalconMStreamSdk constructor", (void *)this, instance_id_);
		...
		if (global_init_result_ == 0) {
			XBLOG_INFO("this=%p uniqueID=%d Media SDK GlobalInit result=%d initialized_now=%s",
				   (void *)this, instance_id_, global_init_result_, initialized_now ? "true" : "false");
		} else {
			XBLOG_ERROR("this=%p uniqueID=%d Media SDK GlobalInit result=%d initialized_now=%s",
				    (void *)this, instance_id_, global_init_result_,
				    initialized_now ? "true" : "false");
		}
	...
	{
		XBLOG_INFO("this=%p uniqueID=%d FalconMStreamSdk destructor", (void *)this, instance_id_);
		disconnect();
#ifdef SESSION_CREATE_ONCE
	...
	{
		XBLOG_INFO("this=%p uniqueID=%d connect controller_id='%s' device_id='%s' broker_address='%s' "
			   "broker_port=%u",
			   (void *)this, instance_id_, controller_id_.c_str(), device_id.c_str(),
			   broker_address.c_str(), broker_port);
		if (broker_address.empty() || device_id.empty()) {
			XBLOG_ERROR("this=%p uniqueID=%d invalid connection settings", (void *)this, instance_id_);
			return false;
		}
		...
		if (!session) {
			XBLOG_ERROR("this=%p uniqueID=%d connect has no active session", (void *)this, instance_id_);
			return false;
		}
		...
		if (session->connectPeerSession(c) != 0) {
			XBLOG_ERROR("this=%p uniqueID=%d connectPeerSession failed for device '%s'", (void *)this,
			    ...
			    const falconm_video_encoder_options &encoder_options = {}) override
	{
		...
		if (!valid_encoder_option(encoder_options.width) || !valid_encoder_option(encoder_options.height) ||
			...
			XBLOG_ERROR("this=%p uniqueID=%d startStreaming rejected invalid encoder options width=%d "
		...
		XBLOG_INFO(
			"this=%p uniqueID=%d startStreaming controller_id='%s' videoSsrc=%u audioSsrc=%u "
			"dataSsrc=%u srtServerPort=%u encoderWidth=%d encoderHeight=%d encoderFps=%d encoderBitrate=%d",
			(void *)this, instance_id_, controller_id_.c_str(), p.srtPushCfg.videoSsrc,
			p.srtPushCfg.audioSsrc, p.srtPushCfg.dataSsrc, p.srtPullCfg.srtServerPort,
			p.videoEncodeCfg.width, p.videoEncodeCfg.height, p.videoEncodeCfg.fps,
			p.videoEncodeCfg.bitrate);
		...
		if (session->startPullStream(device_id_, p) != 0) {
			XBLOG_ERROR("this=%p uniqueID=%d startPullStream failed for device '%s'", (void *)this,
				    instance_id_, device_id_.c_str());
	...
	bool stopStreaming() override
	{
		...
		fpsStartTimeMs_.store(0, std::memory_order_relaxed);
		if (result != 0) {
			XBLOG_ERROR("this=%p uniqueID=%d stopPullStream failed for device '%s'", (void *)this,
				    instance_id_, device_id_.c_str());
		}
		return result == 0;
	}
	...
	bool send(const FalconRequest &request) override
	{
		...
		XBLOG_INFO("this=%p uniqueID=%d sendPeerMessage device='%s' topic='%s' payloadlen=%u payload=%s",
			   (void *)this, instance_id_, device_id_.c_str(), m.topic.c_str(), m.payloadlen,
			   payload_hex.c_str());
...
private:
	...
	bool createSession()
	{
		...
		if (!session) {
			XBLOG_ERROR("this=%p uniqueID=%d failed to create Dragonfly server session", (void *)this,
				    instance_id_);
			return false;
		}
		...
		if (result != 0) {
			XBLOG_ERROR("this=%p uniqueID=%d BLRTCServerSession startSession failed, result=%d",
		...
		XBLOG_INFO("this=%p uniqueID=%d created and started a clean server session", (void *)this,
			   instance_id_);
		return true;
	}
	...
	void destroySession()
	{
		...
		if (result != 0) {
			XBLOG_ERROR("this=%p uniqueID=%d BLRTCServerSession stopSession failed, result=%d",
				    (void *)this, instance_id_, result);
		}
		XBLOG_INFO("this=%p uniqueID=%d stopped and destroyed server session", (void *)this, instance_id_);
	}

	void disconnectSession(bool destroy_session)
	{
		XBLOG_INFO("this=%p uniqueID=%d disconnect controller_id='%s' device_id='%s' broker_address='%s' "
			   "broker_port=%u connected=%s streaming=%s",
			   (void *)this, instance_id_, controller_id_.c_str(), device_id_.c_str(),
			   broker_address_.c_str(), broker_port_,
	...
	{
		XBLOG_INFO("this=%p uniqueID=%d onPeerDevicesRefresh client=%s", (void *)this, instance_id_,
			   client.name.c_str());
	}
	void onPeerConnectStatus(rtcsdk::BLNSPClient &client, int status) override
	{
		XBLOG_INFO("this=%p uniqueID=%d onPeerConnectStatus client=%s status=%d device_id='%s' "
			   "broker_address='%s'",
			   (void *)this, instance_id_, client.name.c_str(), status, device_id_.c_str(),
			   broker_address_.c_str());
		if (disconnecting_) {
			connected_ = false;
			return;
		}
		...
		if (connected_) {
			...
			if (!send(RtcClockRequest{clock.timestamp, clock.timezone, clock.timezone_id})) {
				XBLOG_WARNING("this=%p uniqueID=%d failed to synchronize device RTC", (void *)this,
					      instance_id_);
			}
			...
			if (!startStreaming(kDefaultVideoSsrc, kDefaultAudioSsrc, kDefaultDataSsrc, encoder_options)) {
				XBLOG_ERROR("this=%p uniqueID=%d startStreaming failed after peer connection",
					    (void *)this, instance_id_);
			}
		} else {
			XBLOG_ERROR("this=%p uniqueID=%d peer connection failed, status=%d", (void *)this, instance_id_,
				    status);
			return;
		}
	}
	...
	{
		if (!d) {
			XBLOG_ERROR("this=%p uniqueID=%d onDecodedFrame received null data, ssrc=%u", (void *)this,
				    instance_id_, ssrc);
			return;
		}
		...
		case MEDIA_DATA_TYPE_VIDEO: {
			...
			if (frame_count % kFpsFrameWindow == 0) {
				...
				const double fps = start_ms > 0 && elapsed_ms > 0
							   ...
							   : 0.0;
				XBLOG_INFO("this=%p uniqueID=%d onDecodedVideoFrame ssrc=%u data=%p format=%d "
					   ...
					   streaming_ ? "true" : "false", (void *)d->buffer.get(),
					   (void *)d->bufferObj.get(), d->width, d->height, (long long)d->pts,
					   (long long)d->dts, d->fromNodeId, d->toNodeId, d->rotation,
		...
		case MEDIA_DATA_TYPE_AUDIO:
			...
			return;
		default:
			XBLOG_WARNING("this=%p uniqueID=%d unsupported decoded media type, ssrc=%u type=%d format=%d",
	...
	{
		if (!streaming_) {
			XBLOG_WARNING("this=%p uniqueID=%d decoded video frame rejected because streaming is false, "
				      "ssrc=%u format=%d",
		...
		case MEDIA_DATA_FORMAT_YUV420P:
			...
			return;
		default:
			XBLOG_WARNING("this=%p uniqueID=%d unsupported decoded video format, ssrc=%u format=%d",
				      (void *)this, instance_id_, ssrc, d.format);
			return;
		}
	}
	...
	{
		if (!d.bufferObj) {
			XBLOG_ERROR("this=%p uniqueID=%d IMAGE_BUFFER decoded frame has null bufferObj, ssrc=%u",
		...
		if (!pixel_buffer) {
			XBLOG_ERROR(
				"this=%p uniqueID=%d IMAGE_BUFFER GenericObject returned null CVPixelBuffer, ssrc=%u "
				"bufferObj=%p",
		...
		if (lock_result != kCVReturnSuccess) {
			XBLOG_ERROR("this=%p uniqueID=%d CVPixelBufferLockBaseAddress failed, ssrc=%u "
		...
		case kCVPixelFormatType_420YpCbCr8BiPlanarFullRange:
			...
			break;
		default:
			XBLOG_WARNING("this=%p uniqueID=%d unsupported CVPixelBuffer format 0x%08x", (void *)this,
				      instance_id_, (unsigned)pixel_format);
		...
		if (decoded_cb_) {
			...
			XBLOG_WARNING(
				"this=%p uniqueID=%d IMAGE_BUFFER frame dropped because decoded callback is empty, "
				"ssrc=%u size=%ux%u",
	...
	{
		if (!d.buffer) {
			XBLOG_WARNING("this=%p uniqueID=%d decoded video frame rejected because buffer is null, "
				      "ssrc=%u format=%d",
				      (void *)this, instance_id_, ssrc, d.format);
			return;
		}
		...
		if (!base) {
			XBLOG_WARNING("this=%p uniqueID=%d decoded video frame rejected because buffer data is null, "
		...
		if (size <= 0) {
			XBLOG_WARNING(
				"this=%p uniqueID=%d decoded video frame rejected because buffer size is invalid, "
		...
		if (d.width <= 0 || d.height <= 0) {
			XBLOG_WARNING(
				"this=%p uniqueID=%d decoded video frame rejected because dimensions are invalid, "
		...
		if (decoded_cb_) {
			...
			XBLOG_WARNING(
				"this=%p uniqueID=%d video frame dropped because decoded callback is empty, ssrc=%u "
				"format=%d size=%dx%d",
	...
	{
		if (!streaming_) {
			XBLOG_WARNING("this=%p uniqueID=%d decoded audio frame rejected because streaming is false, "
				      "ssrc=%u format=%d",
				      (void *)this, instance_id_, ssrc, d.format);
			return;
		}
		...
		case MEDIA_DATA_FORMAT_PCM_S16:
			...
			return;
		default:
			XBLOG_WARNING("this=%p uniqueID=%d unsupported decoded audio format, ssrc=%u format=%d",
				      (void *)this, instance_id_, ssrc, d.format);
			return;
		}
	}
	...
	{
		if (!d.buffer) {
			XBLOG_WARNING("this=%p uniqueID=%d decoded audio frame rejected because buffer is null, "
				      "ssrc=%u format=%d",
				      (void *)this, instance_id_, ssrc, d.format);
			return;
		}
		...
		const int size = d.buffer->getSize();
		if (!base) {
			XBLOG_WARNING("this=%p uniqueID=%d decoded audio frame rejected because buffer data is null, "
				      "ssrc=%u format=%d buffer=%p size=%d",
				      (void *)this, instance_id_, ssrc, d.format, (void *)d.buffer.get(), size);
			return;
		}
		if (size <= 0) {
			XBLOG_WARNING(
				"this=%p uniqueID=%d decoded audio frame rejected because buffer size is invalid, "
				"ssrc=%u format=%d data=%p size=%d",
				(void *)this, instance_id_, ssrc, d.format, (void *)base, size);
			return;
		}
		...
		if (audio_cb_) {
			...
			XBLOG_WARNING(
				"this=%p uniqueID=%d audio frame dropped because audio callback is empty, ssrc=%u "
	...
	void onPeerMessage(rtcsdk::BLNSPClient &client, const rtcsdk::MQTTMessage &m) override
	{
		...
		XBLOG_INFO("this=%p uniqueID=%d onPeerMessage client='%s' topic='%s' payloadlen=%u payload=%s",
			   (void *)this, instance_id_, client.name.c_str(), m.topic.c_str(), m.payloadlen,
			   payload_hex.c_str());
		if (!m.payload && m.payloadlen != 0) {
			XBLOG_ERROR("this=%p uniqueID=%d invalid signaling payload from '%s'", (void *)this,
		...
		if (factory != event_factories_.end()) {
			...
			if (event->parse(payload, m.payloadlen)) {
				...
				XBLOG_WARNING("this=%p uniqueID=%d invalid %s payload, size=%u hex=%s", (void *)this,
	...
	{
		XBLOG_INFO("this=%p uniqueID=%d [socket_source] new srt stream ssrc=%u type=%d format=%d", (void *)this,
			   instance_id_, stream.ssrc, stream.mediaType, stream.mediaFormat);
		...
		auto session = sessionSnapshot();
		if (!session) {
			XBLOG_WARNING("this=%p uniqueID=%d new SRT stream ignored because session is null",
				      (void *)this, instance_id_);
			return;
		}
		if (stream.mediaType == blink::media::MEDIA_DATA_TYPE_VIDEO) {
			...
			XBLOG_INFO("this=%p uniqueID=%d [socket_source] add video surface ssrc=%u output=%dx%d",
	...
	{
		XBLOG_INFO("this=%p uniqueID=%d onDeleteSrtStream ssrc=%u", (void *)this, instance_id_, stream.ssrc);
		auto session = sessionSnapshot();
		...
		XBLOG_WARNING("this=%p uniqueID=%d SRT stream deleted, ssrc=%u", (void *)this, instance_id_,
			      stream.ssrc);
	}
	void onVideoFormatChanged(uint32_t ssrc, int width, int height) override
	{
		XBLOG_INFO("this=%p uniqueID=%d onVideoFormatChanged ssrc=%u size=%dx%d", (void *)this, instance_id_,
			   ssrc, width, height);
	}
	void onSrtPullStates(const SrtPullStatesMessage &state) override
	{
		// XBLOG_INFO("this=%p uniqueID=%d onSrtPullStates quality=%d loss=%.2f%%", (void *)this,
		//                  instance_id_, state.networkQualityLevel, state.packetLossRate * 100.0);
		if (state.networkQualityLevel == 0) {
			XBLOG_WARNING("this=%p uniqueID=%d SRT network quality is lowest, loss=%.2f%%", (void *)this,
				      instance_id_, state.packetLossRate * 100.0);
		}
	}
